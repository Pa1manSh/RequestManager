...
<template>
  ...
  <div class="content-interview" v-else-if="interview?.id && !isLoading">
    <app-card>
      <template #title>Заказ на разработку от {{ interview.company }}</template>
      <template #content>
        ...
        <div class="flex flex-column gap-2">
          <label for="vacancyLink">Описание заказа (ссылка)</label>
          <app-input-text
            class="input mb-3"
            id="vacancyLink"
        ...
        <div class="flex flex-column gap-2">
          <label for="hrName">Имя заказчика</label>
          <app-input-text
            class="input mb-3"
            id="hrName"
            v-model="interview.customersName"
          />
        </div>
        <div class="flex flex-column gap-2">
